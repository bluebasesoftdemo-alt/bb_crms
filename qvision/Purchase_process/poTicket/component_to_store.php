...
$update_sts = $con->query("UPDATE po_ticket SET status= 3 WHERE id='$taskid'"); //BOM Verification   /// Status = 3 ////

$con->query("INSERT INTO `bom_component`(`cost_sheet_id`, `po_ticket_id`, `bom`, `component_remark`,`status`, `created_by`) VALUES ('$csId','$taskid','$bom','$components ',0,'$candidateid')");


//$update_grn_sts = $con->query("UPDATE purchase_generate SET status= 1 WHERE id = '$purchaseId'"); //After Customization Completed  Request to Finance for raising Invoice /// Status = 2 ////
