...
<div class="tab-content">
    ...
    <table class="table table-bordered">
        ...
        <td>Client Name</td>
        <td colspan="5">
		<select required aria-required="true" class="form-control" id="txt_org_name" name="txt_org_name" onchange="plantt(this.value)"> 
			<option value="">Choose Org Name</option>
			<?php $stmt3 = $con->query("SELECT id,org_type,org_name FROM new_client_master");
			while ($row3 = $stmt3->fetch()) { ?>
			<option value="<?php echo $row3['id']; ?>"> <?php echo $row3['org_name']; ?></option>
			<?php }  ?>
		</select>
		</td>
        </tr>
        ...
        <td colspan="5">
		<select required aria-required="true" class="form-control" id="plant" name="plant" > 
...
var formData = new FormData(this);
           $.ajax({  
                 url:"qvision/receivable_payable/receivable/receivable_submit.php",
                method:"POST",  
                data:formData, 
				cache: false,
				contentType: false,
...
function plantt(v){
 $.ajax({
type:"GET",
url:"qvision/receivable_payable/receivable/plant_fetch.php?idd="+v,
success:function(data){
$("#plant").html(data);
}
})	
